    ...
    module ClassMethods
      ...
      def verify_email!(email_validation_key)
        if user = find_by_email_validation_key(email_validation_key)
          ...
          raise UserVerificationError, 'We were not able to verify your account or it may have been verified already. Please contact us for assistance.'
        end
      end
    
      def roles_for_select
        self.wristband[:roles].collect{|k| [k.to_s.gsub('_', ' '), k.to_s]}
      end
